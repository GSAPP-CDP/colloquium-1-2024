...
map.on("load", function () {
    ...
    map.addLayer({
        ...
        'source': {
            'type': 'geojson',
            'data': './tree_condition/tree_whole.geojson'
        },
        'paint': {
            'fill-color': [
                'interpolate', ['linear'], ['get', 'tree population change'],
                0,'#ffffff',
                10, '#e0e9dc',
                256, '#2d631c'
            ],
    ...
    map.addLayer({
        'id': 'tree_condition_1995',
        'type': 'fill',
        'source': {
            'type': 'geojson',
            'data': './tree_condition/tree condition.geojson'
        },
        'paint': {
            'fill-color': [
                'interpolate', ['linear'], ['get', 'good_condition_percentage_1995'],
                0.35, '#ffffff',
                1, '#244ca3'
            ],
            'fill-opacity': 1,
    ...
    map.addLayer({
        ...
        'paint': {
            'fill-color': [
                'interpolate', ['linear'], ['get', 'good_condition_percentage_2015'],
                0.35, '#ffffff',
                1, '#244ca3' 
            ],
            'fill-opacity': 1
    ...
    map.addLayer({
        'id': 'building area',
        'type': 'fill',  // 使用 fill-extrusion 类型
        'source': {
            'type': 'geojson',
            'data': './tree_condition/hexagons_whole.geojson'
        },
        'paint': {
            'fill-color': [
                'interpolate', ['linear'], ['get', 'building_area_sum'],
                0, '#fcefda',
                0.291752091659514e-06, '#d25304' 
            ],
            'fill-opacity': 1
        }
    }, 'bridge-rail');
    ...
    map.addLayer({
        ...
        'paint': {
            'fill-extrusion-color': [
                'interpolate', ['linear'], ['get', 'difference of condition'],
                -1, '#8b332d',
                -0.5, '#dc6565',
                0, '#f5f5f5',
                0.5, '#9abada',
            ...
            'fill-extrusion-opacity': 0.9, // 控制透明度
            'fill-extrusion-base': 0 // 设置基准高度
        }
    }, 'bridge-simple');

    map.addLayer({
        'id': 'building number difference',
        'type': 'fill-extrusion',  // 使用 fill-extrusion 类型
        'source': {
            'type': 'geojson',
            'data': './tree_condition/hexagons_whole.geojson'
        ...
        'paint': {
            'fill-extrusion-color': [
                'interpolate', ['linear'], ['get', 'new_building_count'],
                0, '#fbfbee',
                4,'#f3f976',
                10, '#cc8b85',
                80, '#5a008a',
    ...
    map.addLayer({
        ...
        'paint': {
            'fill-extrusion-color': [
                'interpolate', ['linear'], ['get', 'change of line'],
                0, '#ebe64c',
                4,'#77a682',
                20, '#5b7c8b',
                61, '#2f0863',
